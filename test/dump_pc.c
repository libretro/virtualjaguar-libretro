...
int main(int argc, char *argv[]) {
   ...
   num_frames = argc > 2 ? (unsigned)atoi(argv[2]) : 460;
...
#define LOAD(sym) do { p_##sym = dlsym(handle, #sym); if (!p_##sym) { fprintf(stderr, "Missing: %s\n", #sym); return 1; } } while(0)
   ...
   for (f = 0; f < num_frames; f++) {
      p_retro_run();
      if (p_m68k_get_reg) {
         uint32_t cur_pc = p_m68k_get_reg(NULL, 16);
         uint32_t cur_sp = p_m68k_get_reg(NULL, 15);
         if (cur_pc != prev_pc && f >= 400) {
   ...
   sp = p_m68k_get_reg(NULL, 15);
   printf("\n=== Final: PC=$%06X SP=$%06X ===\n", pc, sp);

   /* Dump code around stuck PC */
   base = (pc > 0x40) ? pc - 0x40 : 0;
   printf("\nCode at $%06X-$%06X:\n", base, pc + 0x60);
   for (a = base; a < pc + 0x60 && a < 0x200000; a += 2)
      printf("  $%06X: %02X%02X%s\n", a, ram[a], ram[a+1], (a == pc) ? "  <-- PC" : "");

   /* Dump stack */
   printf("\nStack at SP=$%06X:\n", sp);
   for (a = sp; a < sp + 0x40 && a + 3 < 0x200000; a += 4) {
      uint32_t val32 = (ram[a]<<24)|(ram[a+1]<<16)|(ram[a+2]<<8)|ram[a+3];
      printf("  $%06X: $%08X\n", a, val32);
   }
