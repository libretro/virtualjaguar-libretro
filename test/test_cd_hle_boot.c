...
struct cd_disc_result {
    ...
    /* CD subsystem activity captured AFTER the run.  Lets us distinguish
     * "BIOS wedged in CD spin-loop" from "BIOS booted game code." */
                            ...
                            struct cd_disc_result *out)
{
    ...
    if (!hist.unique_overflow && hist.unique_count <= 32) {
        ...
        if (C.m68k_get_reg) {
            ...
            space = (uint8_t *)dlsym(C.handle, "jagMemSpace");
            a0 = C.m68k_get_reg(NULL, 8);
            a1 = C.m68k_get_reg(NULL, 9);
            if (space) {
                uint32_t k;
                if (a0 < 0xE00000) {
                    fprintf(stderr, "    [A0-MEM $%06X]", a0);
                    for (k = 0; k < 32 && a0 + k < 0xE00000; k++)
                        fprintf(stderr, " %02X", space[a0 + k]);
                    fprintf(stderr, "\n");
                }
                if (a1 < 0xE00000) {
                    fprintf(stderr, "    [A1-MEM $%06X]", a1);
                    for (k = 0; k < 32 && a1 + k < 0xE00000; k++)
                                   ...
                                   struct cd_child_status *status)
{
    ...
    close(pipefd[1]);
    got = read(pipefd[0], &status->result, sizeof(status->result));
    close(pipefd[0]);
    (void)got;
...
TEST(boot_all_discovered_discs)
{
    ...
    for (i = 0; i < discs.count; i++) {
        ...
        ok = r->pc_stayed_in_ram && r->not_thrashing &&
                  r->ram_has_payload;
