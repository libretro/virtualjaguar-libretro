; test/microbench/benchgpu_branch.s -- 68K bootstrap for the GPU
; branch-heavy-loop microbenchmark ROM (#536, task 3).
;
; Identical shape to benchgpu_arith.s (task 2): the 68K's only job is to
; prove the cart booted (START sentinel), copy the GPU program into GPU
; local RAM, kick GPUGO, then spin. All of the measured work -- and the
; DONE/COUNT sentinel writes -- happens on the GPU
; (benchgpu_branch_gpu.s), which can write main RAM directly (STORE to an
; external address is a normal GPU instruction, no 68K involvement
; needed).
;
...
;   rmac -fb -o benchgpu_branch.o benchgpu_branch.s
;   rln  -n -a 800000 x x -o benchgpu_branch.j64 benchgpu_branch.o
;
...
start:
        ...
        ; simplest correct thing for a ~150-byte blob; no alignment
