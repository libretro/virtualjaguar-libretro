/*
 * test/tools/test_microbench_gpu_branch.c -- assertion harness for the GPU
 * branch-heavy-loop microbenchmark ROM
 * (test/microbench/benchgpu_branch.j64, issue #536).
 *
 * Mirrors test_microbench_gpu_arith.c exactly except for the ROM, the DONE
 ...
 *
 *   MICROBENCH engine=gpu_branch done=<0|1> done_frame=<N|-1> \
 *       start_frame=<N|-1> count=<iterations> expect_count=<iterations> \
 *       budget=<frames>
 *
 * `done_frame` is the point of the exercise: the frame the GPU's
 * branch-heavy loop finished on.
 *
 * Needs the wide test ABI (make TEST_EXPORTS=1) for the jaguarMainRAM
 * export -- same as test_microbench_gpu_arith.c.
 *
 * Build:
 *   cc -O2 -Wall -std=c99 -I. -I./src -I./libretro-common/include \
 *      -o test/tools/test_microbench_gpu_branch \
 *      test/tools/test_microbench_gpu_branch.c \
 *      test/harness/harness.c -ldl -lm
 *
 * Run:
 *   ./test/tools/test_microbench_gpu_branch ./virtualjaguar_libretro.dylib \
...
#define MB_MAGIC_GPUBR      0xC0DE0B02u

/* benchgpu_branch_gpu.s: ITERATIONS .equ 2000000 */
#define MB_EXPECT_COUNT     2000000u

/* Measured completion frame, harness defaults (dram_timing /
 * gpu_pipeline_timing both "disabled").  600 matches bench68k.j64's and
 * benchgpu_arith.j64's budget -- see test/microbench/README.md's
 * frame-budget table for why that clears every measured timing-model
 * configuration with margin.  If this ROM's ITERATIONS ever changes,
 * re-measure and update both this comment and MB_FRAME_BUDGET together. */
                        ...
                        unsigned width, unsigned height, size_t pitch)
{
    ...
    if (st->done_frame < 0 && mb_read32(ram, MB_SENT_DONE) == MB_MAGIC_GPUBR) {
...
int main(int argc, char **argv)
{
    ...
    harness_result res;
    int failed = 0;
    const char *detail = "GPU branch-heavy loop completed with the expected iteration count";
    ...
    if (!st.ramp) {
        fprintf(stderr,
                "test_microbench_gpu_branch: core does not export "
    ...
    if (!harness_load_rom(&cfg)) {
        fprintf(stderr, "test_microbench_gpu_branch: failed to load ROM \"%s\"\n",
    ...
    printf("MICROBENCH engine=gpu_branch done=%d done_frame=%d start_frame=%d "
    ...
    res.name   = "microbench_gpu_branch";
    res.detail = detail;
