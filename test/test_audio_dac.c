...
TEST(pit_timer_rate_calculation)
{
    ...
    ps = jerry_read(JERRY_JPIT1_R);
    dv = jerry_read(JERRY_JPIT2_R);
    cycles = ((uint32_t)ps + 1) * ((uint32_t)dv + 1);
...
TEST(dac_i2s_rate_from_sclk)
{
    ...
    sclk_val = 8;
    i2s_cycles = 32 * (2 * (sclk_val + 1));
    actual_rate = (double)RISC_CLOCK_NTSC / (double)i2s_cycles;
...
TEST(wavetable_rom_triangle_accessible)
{
    ...
    hi = jerry_read(ROM_TRI);
    lo = jerry_read(ROM_TRI + 2);
...
TEST(wavetable_rom_triangle_symmetry)
{
    ...
    s0 = jerry_read(ROM_TRI);
    s64 = jerry_read(ROM_TRI + 64 * 2);
    ASSERT_TRUE(s0 != s64);
}
...
TEST(sclk_default_rate)
{
    ...
    default_sclk = 19;
    cycles_per_sample = 32 * 2 * (default_sclk + 1);
    ASSERT_EQ(cycles_per_sample, 1280);
    rate = (double)RISC_CLOCK_NTSC / (double)cycles_per_sample;
...
TEST(sclk_cd_quality_rate)
{
    ...
    sclk_val = 8;
    cycles_per_sample = 32 * 2 * (sclk_val + 1);
    rate = (double)RISC_CLOCK_NTSC / (double)cycles_per_sample;
...
TEST(i2s_timing_usec_calculation)
{
    ...
    risc_usec = 0.03760684198;
    cycles_8 = 32 * (2 * (8 + 1));
    usec_8 = (double)cycles_8 * risc_usec;
    rate_8 = 1000000.0 / usec_8;
    ASSERT_TRUE(rate_8 > 44000.0 && rate_8 < 47000.0);

    cycles_19 = 32 * (2 * (19 + 1));
    usec_19 = (double)cycles_19 * risc_usec;
    rate_19 = 1000000.0 / usec_19;
