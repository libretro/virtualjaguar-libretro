...
/* Track unique diff command patterns */
...
static void BlitterRunComparison(void)
{
	...
	/* Log initial register state for first few GOURD blits */
	...
	/* Save destination region and blitter state */
	...
	if (diff_bytes > 0)
	{
		...
		if (blit_cmp_logged < BLIT_CMP_MAX_LOG)
		{
			...
			/* Show first few differing pixels for 16bpp */
