...
> **Stamp provenance for this sweep.** All 20 rows below are stamped
> `17375c3-dirty` -- the tree carried the (then-uncommitted)
> `cd_boot_matrix.sh` fix while they were measured, so that is the honest
> build id for the code under test. They are *not* stamped with the commit
> that contains them, and cannot be: the stamp is computed once per
> invocation (script line 110) from `scripts/build-id.sh`, and this file is
> tracked, so the moment a chunked sweep writes its first row the tree goes
> dirty and the *next* invocation computes a different id and re-runs
> everything it just recorded. **A chunked sweep can therefore never
> converge on a clean stamp** -- only a single unlimited invocation
> (`CD_MATRIX_MAX_RUNS=0`) from a clean tree can, and even then the commit
> that lands the table necessarily has a different sha than the stamp.
> Consequence for the next reader: a fresh run will see these rows as
> stale and re-measure all 20. That is the guard behaving as designed on an
> unavoidable provenance gap, not evidence of a bad row.
