...
Source: Jaguar Software Reference Manual v2.4 p.79 "Interrupts" (DSP interrupt
table: `#2` "Timer interrupt 1", `#3` "Timer interrupt 2"; vector = 16 x
interrupt number per the GPU Interrupts section p.39) and p.71 JINTCTRL table
(`docs/atari-jaguar-1999/03 - Software Reference.pdf`); implementation
`src/jerry/jerry.c` (`JERRYPIT1Callback`, `JERRYPIT2Callback`).
...
Source: Jaguar Software Reference Manual v2.4 p.14 INT1 register table
(`docs/atari-jaguar-1999/03 - Software Reference.pdf`) -- bit 4 `C_JERENA`
"Jerry: This interrupt is generated by an input to Tom and is intended for
use by Jerry. This is an active high edge-triggered interrupt"; implementation
`src/jerry/jerry.c`, `src/tom/tom.h` (`IRQ_DSP` enum).
...
Derived from: `src/jerry/jerry.c` -- NOT verified against the JTRM
...
Derived from: `src/jerry/jerry.c` (`JERRYI2SCallback`), `src/jerry/dac.c`,
`src/jerry/dac.h`, `libretro.c` (audio callback) -- NOT verified against the
JTRM
...
Derived from: `src/jerry/joystick.c` -- NOT verified against the JTRM
...
Derived from: `src/jerry/eeprom.c` -- NOT verified against the JTRM
...
Derived from: `src/jerry/dsp.c`, `src/jerry/dsp.h`, `docs/jtrm-gpu-dsp.md` --
NOT verified against the JTRM
