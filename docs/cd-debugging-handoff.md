...
0b. **DSA back-to-back command response overwrite (device-traced on Baldies bios, 2026-07-15).** After transfer #2 completes, Baldies sends `$0200` (Stop), then `$7001` (Set DAC Mode) and `$150A` (Set Mode) **in the same tick**; our single `cdCmd` latch (cdrom.c ~line 951) lets `$150A` clobber the pending `$7001` echo. The game polls DS_DATA three times, receives `$170A` each time, never sees the `$70nn` echo, and wedges (GPU spin at `$F0305E`, cd_seek_wedge with drains frozen at 45896, seeks stuck at 2/2 — no new play command ever issued). Fix direction: queue DSA responses (the seek/stop queue already exists) instead of a single latch. Likely the same mechanism behind other post-transfer stalls where a game interleaves mode/DAC commands.
