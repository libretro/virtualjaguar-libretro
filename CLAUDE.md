...
- `src/core/titledb.c` — per-title enhancement defaults (#368); applied at option-read time in `libretro.c`, user-set values always win. Also carries `negative[]` (#464): known-bad `{key, value}` entries that refuse an unsafe per-title *default* (with a warning) but never override an explicit user choice (warn-and-honour instead). Ships with **zero** negative rows — same "mechanism, not the data" precedent as `titlehook.c`; evidence bar + refuse/warn contract in [`docs/enhancement-hooks.md`](docs/enhancement-hooks.md#negative--known-bad-entries-issue-464)
